package com.ocd.amygdala.ui.fragment.signUp
...
import com.ocd.amygdala.utils.MyApplication
...
class SignUpViewModel(private val myApplication: Application) : AndroidViewModel(myApplication) {
    ...
    private fun createUserWithImage() {
        ...
        val userImage = data.child("images/" + auth.currentUser!!.uid + ".png")
        userImage.putFile(this.imageUri!!).addOnProgressListener {
    ...
    private fun saveUserToDatabase(user: FirebaseUser, items: HashMap<String, Any>) {
        ...
        database.collection("users").document(user.uid).set(items)
            .addOnSuccessListener {
                ...
                userModel.userImagePATH = items["profilePictureURL"].toString()
